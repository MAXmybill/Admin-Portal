...
export default function Sidebar({ isOpen, setIsOpen }) {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed inset-y-0 left-0 z-40 w-64 bg-[#1E255E] text-white flex flex-col transition-transform duration-300 ease-in-out shadow-2xl lg:shadow-none ${
      ...
      >
        ...
        <div className="h-16 flex items-center justify-between px-6 border-b border-indigo-900/60 bg-[#171D4B]">
          <div className="flex items-center space-x-3">
            <img src="/logo.png" alt="MAXmybill Logo" className="h-12 object-contain" />
            <div>
              <p className="text-[10px] text-indigo-300 tracking-widest font-semibold uppercase mt-0.5">Company Owner Admin</p>
            </div>
          </div>
          <button className="lg:hidden p-1 text-indigo-300 hover:text-white rounded-lg hover:bg-white/10" onClick={() => setIsOpen(false)}>
            <X className="w-5 h-5" />
          </button>
        </div>
